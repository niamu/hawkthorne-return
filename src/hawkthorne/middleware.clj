(ns hawkthorne.middleware
  "This was lifted entirely and shamelessly from @anmonteiro
  https://github.com/anmonteiro/talks"
...
(defn wrap-transit-body
  "Middleware that parses the body of Transit request maps, and replaces the
  :body key with the parsed data structure. Requests without a Transit content
  type are unaffected.
    ...
    (fn [request]
      (let [response (handler request)]
        (if (coll? (:body response))
          (let [transit-response (update-in response [:body]
                                            write encoding opts)]
